...
- 能够稳定收敛，但后期提升趋缓，测试集准确率约在 **0.86~0.87** 区间（以本地日志为准）。

### 阶段 2：训练策略与结构初步优化
- 在基础模型上尝试数据增强、正则化、学习率策略、验证集驱动训练等改进。
- 指标有小幅提升，但增益有限（约 <2%），说明仅靠轻量调参难以突破容量上限。
- 该阶段的价值在于：定位瓶颈主要来自模型表达能力，而非单纯训练技巧不足，为后续方案选择提供依据。

### 阶段 3：升级为 ResNet18（最终方案）
- 将主干网络升级为 ResNet18，并对 CIFAR-10 输入做适配（首层卷积与池化调整）。
- 结合训练策略后，收敛更稳定，最终测试集准确率达到 **0.931**。
- 验证集准确率长期稳定在 **0.93~0.94** 区间，说明模型具有较好的泛化能力。
